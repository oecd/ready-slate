...
const blockTagMatcher = /<(p|div|h1|h2|ul|ol|li)\b/i;

export const insertPredefinedText = (editor, rawText) => {
  if (!editor.selection) {
    return;
  }

  const text = R.defaultTo('', rawText);
  ...
  const parsedNodes = htmlStringToSlate(text);

  // Keep insertion inline when content does not contain block tags.
  if (!blockTagMatcher.test(text)) {
    const firstNode = R.head(parsedNodes);

    if (
      firstNode &&
      firstNode.type === blockTypes.paragraph &&
      R.is(Array, firstNode.children)
    ) {
      Transforms.insertFragment(editor, firstNode.children);
      return;
    }

    Transforms.insertText(editor, slateToString(parsedNodes));
    return;
  }
