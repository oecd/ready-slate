...
const PredefinedTextsButton = ({ predefinedTexts = [] }) => {
  ...
  const isGroupItem = (item) =>
    R.is(Array, item?.items) && !R.isEmpty(item.items);
  const isLeafItem = (item) => R.is(String, item?.text);
  const isPathPrefix = (prefix, path) =>
    R.all(
      (index) => prefix[index] === path[index],
      R.range(0, R.length(prefix)),
      ...
      {items.map((item, index) => {
        ...
        if (isSeparatorItem(item)) {
          return (
            <div
              key={`sep-${pathKey}`}
              className="rs-predefined-menu-separator"
            />
          );
        }
        ...
        if (hasChildren) {
          return (
            ...
            >
              ...
              <FontAwesomeIcon
                icon={faChevronRight}
                className="rs-predefined-menu-item-arrow"
              />
              {shouldShowSubmenu && (
  ...
  return (
    <div
      className="rs-predefined-menu-wrapper"
      ref={menuRef}
      title="Insert predefined texts"
    >
      <Button
        onClick={(e) => {
          e.preventDefault();
      ...
      {open && (
        <div className="rs-predefined-menu">
          {renderMenuItems(predefinedTexts)}
        </div>
      )}
    </div>
  );
